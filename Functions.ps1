# Fonction d'envoi de mail (avec image, pj, etc.)
function Send-MailMessageFonction {
    [CmdletBinding()]
    param(
    ...
    begin
    {
        ...
        try
        {
            ...
            if ($BodyAsHtml)
            {
                $_bodyPart = [Net.Mail.AlternateView]::CreateAlternateViewFromString($Body, 'text/html; charset=utf-8')
            }
            else
            {
